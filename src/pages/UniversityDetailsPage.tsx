// src/pages/UniversityDetailsPage.tsx
...
interface Props {
  universityId: number; // Теперь ID приходит из родителя
}

export const UniversityDetailsPage: React.FC<Props> = ({ universityId }) => {
  ...
  return (
    <div>
      <h1 style={{ fontSize: '32px', marginBottom: '8px' }}>{uni.name}</h1>
      ...
      <h2 style={{ marginTop: '40px', marginBottom: '16px' }}>Факультеты</h2>
      <div style={{ display: 'grid', gap: '12px' }}>
        {faculties?.map(fac => (
          <FacultyCard 
            ...
            name={fac.name} 
            rating={fac.rating} 
            onClick={() => console.log('Клик по факультету', fac.id)} 
          />
        ))}
      </div>
    </div>
